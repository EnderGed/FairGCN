...
class GinN(GCNBaseN):
    """
    GIN model using DGL, following the original paper, we use sum as the aggregation function and epsilon = 0.
    ...
    def __init__(self, data: dgl.DGLGraph, hid_feats: int, dropout: (float, float) = (0.5, 0.),
        ...
        self.model = GinNNet(data.ndata['feat'].shape[1], hid_feats, data.num_classes, dropout).to(
            self.device).double()
...
class GinNNet(GCNBaseNNet):
    ...
    def __init__(self, in_feats, hid_feats, out_feats, dropout):
        ...
        self.conv0 = GINConv(apply_func=None, aggregator_type='sum')
        self.mlp0 = SimpleMLPNet(in_feats=in_feats, out_feats=hid_feats, layers=2, dropout=0.)
        self.drop0 = torch.nn.Dropout(dropout[0])
        self.conv1 = GINConv(apply_func=None, aggregator_type='sum')
        self.mlp1 = SimpleMLPNet(in_feats=hid_feats, out_feats=hid_feats, layers=2, dropout=0.)
        self.drop1 = torch.nn.Dropout(dropout[1])

    def get_embedding(self, g, h):
        h = self.drop0(F.relu(self.mlp0(self.conv0(g, h))))
        h = self.drop1(F.relu(self.mlp1(self.conv1(g, h))))
        return h
