...
from ModelGCN import EwBaseN, GinNNet
from consts import SEED


class EwGinN(EwBaseN):
    """
    GIN GCN model with edge weightening.

    Be sure to modify this file if you modify GinN - bad practice here, but I'm not sure how to make it better.
    """
    def __init__(self, data: dgl.DGLGraph, alpha: float, hid_feats: int, dropout: (float, float) = (0.5, 0.),
        ...
        self.model = EwGinNNet(data.ndata['feat'].shape[1], hid_feats, data.num_classes, dropout).to(
            self.device).double()
...
class EwGinNNet(GinNNet):

    def get_embedding(self, g, h):
        h = self.drop0(F.relu(self.mlp0(self.conv0(g, h, edge_weight=g.edata['w']))))
        h = self.drop1(F.relu(self.mlp1(self.conv1(g, h, edge_weight=g.edata['w']))))
        return h