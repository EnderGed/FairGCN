...
from ModelGCN import EwBaseN, SageNNet
from consts import SEED


class EwSageN(EwBaseN):
    """
    GraphSAGE GCN model with edge weightening.

    Be sure to modify this file if you modify SageN - bad practice here, but I'm not sure how to make it better.
    """
    ...
    def __init__(self, data: dgl.DGLGraph, alpha: float, hid_feats: int, dropout: (float, float) = (0.5, 0.),
        ...
        self.model = EwSageNNet(data.ndata['feat'].shape[1], hid_feats, data.num_classes, dropout).to(
            self.device).double()
