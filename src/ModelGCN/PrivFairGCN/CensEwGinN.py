...
from ModelGCN import EwGinN, CensGcnN
from consts import SEED


class CensEwGinN(EwGinN, CensGcnN):
    """
    Edge weightening with privacy censoring
    """

    fair_prefix = CensGcnN.fair_prefix + EwGinN.fair_prefix

    def __init__(self, data: dgl.DGLGraph, alpha: float, priv_attr: str, priv_lambda: float, hid_feats: int,
                 dropout: (float, float) = (0.5, 0.), lr: float = 1e-3, weight_decay: float = 0., gpu: str = 'cpu',
                 run_name: str = None, debug_mode: bool = False, seed: int = SEED):
        """
        :param data: dgl.graph,
        :param alpha: float [0, 1] edge weightening strength parameter, 0 - no weights, 1 - complete counterweights
        :param priv_attr: str, name of the privacy sensitive attribute
        ...
        """
        EwGinN.__init__(self, data=data, alpha=alpha, hid_feats=hid_feats, dropout=dropout, lr=lr,
                         weight_decay=weight_decay, gpu=gpu, run_name=run_name, debug_mode=debug_mode,
                         seed=seed)
        CensGcnN.__init__(self, data=self.data, base_net=self.model, priv_attr=priv_attr, priv_lambda=priv_lambda,
                          lr=lr, weight_decay=weight_decay, run_name=run_name, gpu=gpu, debug_mode=debug_mode,
                          seed=seed)
