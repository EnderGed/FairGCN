...
from ModelGCN import EwFlKipfN, CensGcnN, CensFlGcnN
from consts import SEED


class CensEwFlKipfN(CensFlGcnN, EwFlKipfN, CensGcnN):
    """
    Fair learning with Edge weightening and privacy censoring.
    """

    fair_prefix = CensGcnN.fair_prefix + EwFlKipfN.fair_prefix

    def __init__(self, data: dgl.DGLGraph, ew_alpha: float, alpha: float, priv_lambda: float, hid_feats: int,
                 fairnes_type: str = 'par', dropout: (float, float) = (0.5, 0.), lr: float = 0.001,
        ...
        """
        EwFlKipfN.__init__(self, data=data, ew_alpha=ew_alpha, alpha=alpha, fairnes_type=fairnes_type,
